...
class ExpensesLists extends StatefulWidget {
  ExpensesLists({Key? key}) : super(key: key ?? ExpensesListsState.key);

  @override
  ExpensesListsState createState() => ExpensesListsState();
}

class ExpensesListsState extends State<ExpensesLists> {
  static GlobalKey<ExpensesListsState> key = GlobalKey<ExpensesListsState>();
    ...
    try {
      ...
      setState(() {
        categories = typedCategories
            .where((category) => category['category_type'] == 'expenses')
            .map((category) {
  ...
  void addCategory(String category, IconData icon) async {
    ...
    // int iconCodePoint = icon.codePoint;
    // String iconString = iconCodePoint.toString();
    await _categoryService.addCategory(category, "expenses", iconNameString);
    loadCategories();
  }
...
Color getRandomColorFromList(List<List<Color>> colorPairs) {
  var rng = Random();
  int index = rng.nextInt(colorPairs.length);
  List<Color> colorPair = colorPairs[index];
  return rng.nextBool() ? colorPair[0] : colorPair[1];
}
